...
/*I opted for individual print pages. See bookPrintPage, composerPrintPage, compositionPrintPage.
...
if(isset($_REQUEST['printBook'])) {
$printBook = $_REQUEST['printBook'];

if(isset($_REQUEST['printComposition'])) {
$printComposition = $_REQUEST['printComposition'];

if(isset($_REQUEST['compositionID'])) {
$compositionID = $_REQUEST['bocompositionIDokID'];

if(isset($_REQUEST['bookID'])) {
$bookID = $_REQUEST['bookID'];

if(isset($_REQUEST['composerPrintPage'])) {
$composerPrintPage = $_REQUEST['composerPrintPage'];
...
/*Collecting the book information*/
...
_END;
    ...
    $resultGenreQuery = $conn->query($genreQuery);
    /*echo '$selectGenresQuery = ' . $genreQuery . '<br/><br/>';*/


if($debug) {
    if (!$resultGenreQuery) echo("\n Error description genreQuery: " . mysqli_error($conn) . "\n<br/>");
}/*end debug*/

    if ($resultGenreQuery) {     

      $numberOfGenreRows = $resultGenreQuery->num_rows;
      /*Build comma separated list of genres in a string*/

      $genreString= "";

      for ($j = 0 ; $j < $numberOfGenreRows ; ++$j) {
        $row = $resultGenreQuery->fetch_array(MYSQLI_NUM);
...
if($debug) {
    ...
    if ($resultGenDiffQuery) {     
      ...
      $numberOfGenDiffRows = $resultGenDiffQuery->num_rows;

      for ($j = 0 ; $j < $numberOfGenDiffRows ; ++$j) {
...
if($debug) {
  ...
  if ($resultLyricistQuery){
    ...
    for ($j = 0 ; $j < $numberOfLyricistRow ; ++$j){
...
<h4 class="mb-3">Right Click in your browser to print</h4><br/> 
...
<div class="container-fluid bg-light pt-4 pb-5">
  <div class="col-md-4  pb-4 pt-3">
    <h3>What would you like to do with this information?</h3>
    <form action='bookTitleSearch.php' method='post'>
      <input  class="btn btn-secondary mb-3" type='submit' value='Add a new Composition for this Composer'/>
    </form>

    <form action='introPage.php' method='post'>
      <input  class="btn btn-secondary mb-3" type='submit' value='New Library Search'/>
    </form>

    <form action='printOption.php' method='post'>
      <input  class="btn btn-secondary mb-3" type='submit' value='Print'/>
      <input type='hidden' name="compositionID" value='$compositionID'/>
      <input type='hidden' name="bookID" value='$bookID'/>
